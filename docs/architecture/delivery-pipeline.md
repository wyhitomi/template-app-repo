...
flowchart LR
  ...
  tag[Tag vX.Y.Z on main] --> promote[Re-tag sha image as vX.Y.Z]
...
| Tag `vX.Y.Z` (commit must be on `main`) | `prd` | `prd` (image re-tagged `vX.Y.Z`, no rebuild) |
